  ...
  await $.http
    ...
    .then(async ({ body }) => {
      ...
      for await (let item of items) {
          ...
          await $.http
            ...
            .then(({ body }) => {
              ...
              description = isBundles
                ...
                : data.pages[0].data.about.shortDescription;
            });
        }
        ...
        $.info(`🎮 [Epic 限免]  ${item.title}`);
        $.info(`📅️ 日期: ${startDate} - ${endDate}`);
        $.info(`url: ${url}`);
        $.info(`📰 游戏简介: ${description}`);
        $.notify(
          `🎮 [Epic 限免]  ${item.title}`,
      ...
      (h) =>
        (a[h.toLowerCase()] = (a) =>
          (function (a, h) {
            ...
            const d = e.baseURL;
            d && !u.test(h.url || "") && (h.url = d ? d + h.url : h.url),
              h.body &&
                h.headers &&
                !h.headers["Content-Type"] &&
                (h.headers["Content-Type"] =
                  "application/x-www-form-urlencoded");
            const l = (h = { ...e, ...h }).timeout,
              c = {
                onRequest: () => {},
                onResponse: (e) => e,
                onTimeout: () => {},
            ...
            let f, p;
            if ((c.onRequest(a, h), t)) f = $task.fetch({ method: a, ...h });
            else if (s || o || i)
              f = new Promise((e, t) => {
                (i ? require("request") : $httpClient)[a.toLowerCase()](
                  h,
                  (s, o, n) => {
                    s
                      ? t(s)
              ...
              r &&
                (f = new Promise((e, t) => {
                  fetch(h.url, { method: a, headers: h.headers, body: h.body })
                    .then((e) => e.json())
                    .then((t) =>
                      e({
                        statusCode: t.status,
            ...
            const y = l
              ? new Promise((e, t) => {
                  p = setTimeout(
                    () => (
                      c.onTimeout(),
                      t(`${a} URL: ${h.url} exceeds the timeout ${l} ms`)
                    ),
                    l
                  );
                })
              : null;
            return (
              y ? Promise.race([y, f]).then((e) => (clearTimeout(p), e)) : f
